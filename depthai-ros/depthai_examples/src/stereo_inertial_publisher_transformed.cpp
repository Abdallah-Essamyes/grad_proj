...
std::tuple<dai::Pipeline, int, int> createPipeline(bool enableDepth,
    ...
    // Imu - Modified for faster publishing to prevent timestamp lag
    imu->enableIMUSensor(dai::IMUSensor::ACCELEROMETER_RAW, 500);
    imu->enableIMUSensor(dai::IMUSensor::GYROSCOPE_RAW, 400);
    imu->setBatchReportThreshold(1);  // Publish immediately (was 5, causing 12.5ms delay)
    imu->setMaxBatchReports(10);  // Reduced from 20 for lower latency
...
int main(int argc, char** argv) {
    ...
    bool transformImuToBaseFrame;
    std::string imuBaseFrame;
    ...
    node->declare_parameter("imu_base_frame", std::string("oak-d-base-frame"));
    ...
    node->get_parameter("imu_base_frame", imuBaseFrame);
    ...
    // Create converter function - optionally applies axis transformation from
    // IMU sensor frame to robot base frame (ROS REP-103: X=forward, Y=left, Z=up)
    // Rotation matrix R: [[0,0,1],[-1,0,0],[0,-1,0]]
    // ros_x = sensor_z, ros_y = -sensor_x, ros_z = -sensor_y
    std::function<void(std::shared_ptr<dai::IMUData>, std::deque<sensor_msgs::msg::Imu>&)> imuConvertFunc;
    if(transformImuToBaseFrame) {
        imuConvertFunc = [&imuConverter, imuBaseFrame](std::shared_ptr<dai::IMUData> imuData, std::deque<sensor_msgs::msg::Imu>& output) {
            imuConverter.toRosMsg(imuData, output);
            for(auto& msg : output) {
                // Transform linear acceleration
                double ax = msg.linear_acceleration.z;    // ros_x = sensor_z
                double ay = -msg.linear_acceleration.x;   // ros_y = -sensor_x
                double az = -msg.linear_acceleration.y;   // ros_z = -sensor_y
                msg.linear_acceleration.x = ax;
                msg.linear_acceleration.y = ay;
                msg.linear_acceleration.z = az;

                // Transform angular velocity
                double gx = msg.angular_velocity.z;
                double gy = -msg.angular_velocity.x;
                double gz = -msg.angular_velocity.y;
                msg.angular_velocity.x = gx;
                msg.angular_velocity.y = gy;
                msg.angular_velocity.z = gz;

                // Transform covariance: C_ros = R * C * R^T
                // Pre-computed index mapping for R = [[0,0,1],[-1,0,0],[0,-1,0]]
                auto transformCov = [](std::array<double, 9>& cov) {
                    std::array<double, 9> c = cov;
                    cov[0] =  c[8]; cov[1] = -c[6]; cov[2] = -c[7];
                    cov[3] = -c[2]; cov[4] =  c[0]; cov[5] =  c[1];
                    cov[6] = -c[5]; cov[7] =  c[3]; cov[8] =  c[4];
                };
                transformCov(msg.linear_acceleration_covariance);
                transformCov(msg.angular_velocity_covariance);

                // Update frame_id to base frame
                msg.header.frame_id = imuBaseFrame;
            }
        };
        RCLCPP_INFO(node->get_logger(), "IMU axis transformation to base frame '%s' ENABLED (C++)", imuBaseFrame.c_str());
    } else {
        imuConvertFunc = std::bind(&dai::rosBridge::ImuConverter::toRosMsg, &imuConverter, std::placeholders::_1, std::placeholders::_2);
    }
