...
std::tuple<dai::Pipeline, int, int> createPipeline(bool enableDepth,
    ...
    // Limit stereo queue to 5 frames to prevent USB bandwidth starvation of IMU during motion
    xoutDepth->input.setBlocking(false);
    xoutDepth->input.setQueueSize(5);

    xoutImu->setStreamName("imu");
    // CRITICAL: IMU queue unlimited + non-blocking to prevent message loss during motion
    // When camera moves, stereo data spikes but IMU must not drop packets
    ...
    // Configure threshold filter for depth range limiting
    if(enableThresholdFilter) {
        ...
        std::cout << "[DepthAI] Threshold filter ENABLED: " << thresholdFilterMinRange 
                  << "mm - " << thresholdFilterMaxRange << "mm" << std::endl;
    }

    // IMU configuration optimized to prevent message loss during camera motion
    // 200Hz is sufficient for visual-inertial SLAM and reduces USB bandwidth by 50-60%
    imu->enableIMUSensor(dai::IMUSensor::ACCELEROMETER_RAW, 200);
    imu->enableIMUSensor(dai::IMUSensor::GYROSCOPE_RAW, 200);
    imu->setBatchReportThreshold(1);  // Publish immediately to minimize latency
    imu->setMaxBatchReports(5);  // Smaller batches = lower latency, better for high-frequency
    ...
    if(depth_aligned) {
        ...
        // Limit RGB queue to prevent IMU starvation during motion
        xoutRgb->input.setBlocking(false);
        xoutRgb->input.setQueueSize(5);
        ...
        if(enableSpatialDetection) {
            ...
            xoutPreview->input.setQueueSize(3);
            xoutNN->setStreamName("detections");
            xoutNN->input.setBlocking(false);
            xoutNN->input.setQueueSize(3);
        ...
        // XLinkOut - limit queue size to prevent IMU starvation during motion
        xoutLeft->setStreamName("left");
        xoutLeft->input.setBlocking(false);
        xoutLeft->input.setQueueSize(5);
        xoutRight->setStreamName("right");
        xoutRight->input.setBlocking(false);
        xoutRight->input.setQueueSize(5);
        if(rectify) {
...
int main(int argc, char** argv) {
    ...
    int thresholdFilterMinRange, thresholdFilterMaxRange;
    ...
    bool enableSpatialDetection, enableDotProjector, enableFloodLight, enableThresholdFilter;
    bool usb2Mode, poeMode, syncNN;
    double angularVelCovariance, linearAccelCovariance;
    double dotProjectorIntensity, floodLightIntensity;
    bool enableRosBaseTimeUpdate, transformImuToBaseFrame;
    ...
    node->declare_parameter("i_enable_threshold_filter", false);
    node->declare_parameter("i_threshold_filter_min_range", 200);
    node->declare_parameter("i_threshold_filter_max_range", 15000);
    ...
    node->get_parameter("i_enable_threshold_filter", enableThresholdFilter);
    node->get_parameter("i_threshold_filter_min_range", thresholdFilterMinRange);
    node->get_parameter("i_threshold_filter_max_range", thresholdFilterMaxRange);
    
    // Debug logging for threshold filter configuration
    RCLCPP_INFO(node->get_logger(), "========================================");
    RCLCPP_INFO(node->get_logger(), "Threshold Filter Configuration:");
    RCLCPP_INFO(node->get_logger(), "  Enable: %s", enableThresholdFilter ? "TRUE" : "FALSE");
    RCLCPP_INFO(node->get_logger(), "  Min Range: %d mm", thresholdFilterMinRange);
    RCLCPP_INFO(node->get_logger(), "  Max Range: %d mm", thresholdFilterMaxRange);
    RCLCPP_INFO(node->get_logger(), "========================================");
    
    // Debug logging for inline transformation
    RCLCPP_INFO(node->get_logger(), "========================================");
    RCLCPP_INFO(node->get_logger(), "IMU Transformation Configuration:");
    RCLCPP_INFO(node->get_logger(), "  transform_imu_to_base_frame: %s", transformImuToBaseFrame ? "TRUE" : "FALSE");
    RCLCPP_INFO(node->get_logger(), "  tf_prefix: %s", tfPrefix.c_str());
    RCLCPP_INFO(node->get_logger(), "========================================");
    ...
    // Large host-side queue for IMU to buffer during processing spikes
    // At 200Hz, 200 messages = 1 second buffer
    ...
    // Decide frame_id based on transformation parameter
    std::string imuFrameId = transformImuToBaseFrame ? (tfPrefix + "-d-base-frame") : (tfPrefix + "_imu_frame");
    RCLCPP_INFO(node->get_logger(), "IMU frame_id will be: %s", imuFrameId.c_str());
    
    dai::rosBridge::ImuConverter imuConverter(imuFrameId, imuMode, linearAccelCovariance, angularVelCovariance);
    if(enableRosBaseTimeUpdate) {
        imuConverter.setUpdateRosBaseTimeOnToRosMsg();
    }
    
    // Track if transformation is happening (for debug)
    std::atomic<int> transformCount{0};
    
    // Create conversion function with optional inline transformation
    // MY EDIT!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    dai::rosBridge::BridgePublisher<sensor_msgs::msg::Imu, dai::IMUData> imuPublish(
        ...
        std::string("imu"),
        imuConversionFunc,
        200,  // Increased from 100 to 200 for high-rate IMU
        "",
        "imu");
