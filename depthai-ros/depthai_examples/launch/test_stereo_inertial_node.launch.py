...
def generate_launch_description():
    ...
    transformImuToBaseFrame = LaunchConfiguration('transform_imu_to_base_frame', default = False)  # Default: no inline transform
    imuBaseFrame       = LaunchConfiguration('imu_base_frame', default = 'oak-d-base-frame')

    enableThresholdFilter = LaunchConfiguration('i_enable_threshold_filter', default = False)
    thresholdFilterMaxRange = LaunchConfiguration('i_threshold_filter_max_range', default = '15000')
    ...
    declare_transformImuToBaseFrame_cmd = DeclareLaunchArgument(
        'transform_imu_to_base_frame',
        default_value=transformImuToBaseFrame,
        description='Transform IMU data from sensor frame to base frame (ROS REP-103).')

    declare_imuBaseFrame_cmd = DeclareLaunchArgument(
        'imu_base_frame',
        default_value=imuBaseFrame,
        description='Target base frame for IMU transformation.')

    declare_enable_threshold_filter = DeclareLaunchArgument(
        'i_enable_threshold_filter',
        default_value='false',
        description='Enable stereo post-processing threshold filter on device (min/max range clipping in mm)')

    declare_threshold_filter_max_range = DeclareLaunchArgument(
        'i_threshold_filter_max_range',
        default_value='15000',
        description='Max range for threshold filter (mm)')
    ...
    stereo_node = launch_ros.actions.Node(
            ...
            parameters=[{'mxId':                    mxId},
                        ...
                        {'transform_imu_to_base_frame': transformImuToBaseFrame},
                        {'imu_base_frame':              imuBaseFrame},
                        {'i_enable_threshold_filter':   enableThresholdFilter},
                        {'i_threshold_filter_max_range': thresholdFilterMaxRange}
                        ])
    ...
    ld.add_action(declare_transformImuToBaseFrame_cmd)
    ld.add_action(declare_imuBaseFrame_cmd)
    ld.add_action(declare_enable_threshold_filter)
    ld.add_action(declare_threshold_filter_max_range)
