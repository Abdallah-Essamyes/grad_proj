...
def create_nav2_path(yaml_path, start_px, goal_px, action, topic, frame, tolerance):
    try:
    ...
    def clamp_world_to_map(world_xy, origin, resolution, w, h):
        ox, oy, _ = origin
        mx = int((world_xy[0] - ox) / resolution)
        my = int((world_xy[1] - oy) / resolution)
        cmx = max(0, min(mx, w - 1))
        cmy = max(0, min(my, h - 1))
        # convert back to world (center of cell)
        wx = ox + (cmx + 0.5) * resolution
        wy = oy + (cmy + 0.5) * resolution
        return (wx, wy), (cmx, cmy)

    # clamp goal and start if necessary, keeping tolerance margin
    tol = float(tolerance)
    ...
    if not (0 <= start_idx[0] < width and 0 <= start_idx[1] < height):
        ...
        # also ensure within tolerance margin
        start_m, start_idx = clamp_world_to_map_with_tol(start_m, origin, res, width, height, tol)
    ...
    try:
        ...
        print("rclpy or nav2_msgs not available; computed coordinates:\n")
    ...
    node = Node("compute_nav2_plan_action_client")
    ...
    start.header.stamp = node.get_clock().now().to_msg()
    ...
    try:
        ...
        payload_dict = {
            'start': {
                ...
                'pose': {
                    'position': {'x': float(start_m[0]), 'y': float(start_m[1]), 'z': 0.0},
                    'orientation': {'w': 1.0}
                }
            },
            'goal': {
                'header': {'frame_id': frame},
                'pose': {
                    'position': {'x': float(goal_m[0]), 'y': float(goal_m[1]), 'z': 0.0},
                    'orientation': {'w': 1.0}
                }
            },
        ...
        # fallback to a safely formatted string using .format() and doubled braces
        fallback_template = (
            "{{start: {{header: {{frame_id: '{frame}'}}, pose: {{position: {{x: {sx:.6f}, y: {sy:.6f}, z: 0.0}}, orientation: {{w: 1.0}}}}}, "
            "goal: {{header: {{frame_id: '{frame}'}}, pose: {{position: {{x: {gx:.6f}, y: {gy:.6f}, z: 0.0}}, orientation: {{w: 1.0}}}}}, tolerance: {tol:.6f}}}"
        )
        yaml_payload = fallback_template.format(frame=frame, sx=start_m[0], sy=start_m[1], gx=goal_m[0], gy=goal_m[1], tol=float(tolerance))
    ...
    # result should contain a Path in result.path
    ...
    # If the planner returns an empty path, report unreachable target
